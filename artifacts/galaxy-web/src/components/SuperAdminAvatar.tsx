...
export default function SuperAdminAvatar({ src, userId, size = 90, onClick }: SuperAdminAvatarProps) {
  ...
  return (
    <div className="sa-avatar-wrapper" style={{ width: size, height: size }} onClick={onClick}>
      ...
      {src?.startsWith?.("http") ? (
        <img src={src} alt="" className="sa-avatar-img" onError={e => { (e.target as HTMLImageElement).style.display = "none"; (e.target as HTMLImageElement).parentElement!.textContent = "\u{1F464}"; }} />
      ) : (
        <div className="sa-avatar-img" style={{ display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.35, background: "linear-gradient(135deg, rgba(108,92,231,0.25), rgba(108,92,231,0.1))" }}>{src && src.length <= 4 ? src : "\u{1F464}"}</div>
      )}
